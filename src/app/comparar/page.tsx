...
import { 
  ...
  Briefcase, Clock, Ban, ShieldCheck, Zap, Filter, Calendar, UserCheck, Landmark
} from 'lucide-react';
import { 
...
export default function ComparadorPage() {
  ...
  const CategoryBtn = ({ id, label, icon: Icon }: { id: CategoryType, label: string, icon: any }) => (
    <button
      ...
      className={`flex items-center gap-2 px-6 py-3 rounded-full text-xs font-black uppercase tracking-widest transition-all border-2 ${
        activeCategory === id 
  ...
  return (
    <div className="space-y-6 animate-fadeIn max-w-6xl mx-auto pb-20 px-4 md:px-0">
      ...
      <div className="flex flex-col md:flex-row gap-6 items-end bg-slate-50/50 p-6 rounded-ux border border-slate-100">
        ...
        <div className="hidden md:flex h-12 items-center justify-center text-slate-300 font-black text-xl italic">VS</div>
      ...
      <div className="flex flex-wrap gap-2 py-2 sticky top-0 bg-slate-50/80 backdrop-blur-md z-40 border-b border-slate-100 -mx-4 md:mx-0 px-4 md:px-0">
      ...
      <div className="bg-white rounded-ux border border-slate-200 shadow-xl overflow-hidden relative mt-2">
        ...
        <div className="divide-y divide-slate-100">
          {/* Header de la Tabla con mayor Z-index */}
          <div className="grid grid-cols-1 md:grid-cols-12 bg-slate-50 border-b border-slate-200 sticky top-[72px] z-30">
             <div className="md:col-span-4 p-6 flex items-center font-black uppercase tracking-widest text-[10px] text-slate-400">Concepto</div>
             <div className="md:col-span-4 p-6 flex items-center justify-center gap-3 border-x border-slate-200">
                {getBankLogo(bank1) && <img src={getBankLogo(bank1)!} className="h-6 w-auto grayscale opacity-50" alt="logo" />}
                <span className="font-black uppercase tracking-widest text-[10px] text-slate-600 truncate">{entidades.find(e => e.codigo === bank1)?.nombre}</span>
             </div>
             <div className="md:col-span-4 p-6 flex items-center justify-center gap-3">
                {getBankLogo(bank2) && <img src={getBankLogo(bank2)!} className="h-6 w-auto grayscale opacity-50" alt="logo" />}
                <span className="font-black uppercase tracking-widest text-[10px] text-slate-600 truncate">{entidades.find(e => e.codigo === bank2)?.nombre}</span>
             </div>
          </div>
...
const Row = ({ label, val1, val2, unit='', isPrice=false, isLowerBetter=false, icon:Icon, noBetter=false, isSmall=false }: any) => {
  ...
  const isV1Better = !noBetter && (isLowerBetter ? v1 < v2 : v1 > v2) && val1 !== undefined && val2 !== undefined;
  const isV2Better = !noBetter && (isLowerBetter ? v2 < v1 : v2 > v1) && val1 !== undefined && val2 !== undefined;
  
  const safeL = (v:any) => v != null && typeof v === 'number' ? v.toLocaleString('es-AR') : v;

  return (
    <div className="grid grid-cols-1 md:grid-cols-12 gap-0 items-stretch hover:bg-slate-50/50 transition-all group">
      <div className="md:col-span-4 p-6 flex items-center gap-4 border-r border-slate-100">
        <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-blue-600 group-hover:text-white transition-all"><Icon size={16} /></div>
        <span className="font-display font-bold text-base text-slate-900 leading-tight">{label}</span>
      </div>
      
      <div className={`md:col-span-4 p-6 flex items-center justify-between border-r border-slate-100 transition-all ${isV1Better ? 'bg-blue-50/50' : ''}`}>
        <div className="flex flex-col">
           <span className={`${isSmall ? 'text-[10px]' : 'text-xl'} font-display font-black text-slate-900 tabular-nums`}>
             {val1 === undefined || val1 === null ? 'N/A' : (isPrice ? `$${safeL(val1)}` : `${safeL(val1)}${unit}`)}
           </span>
           {isV1Better && <span className="text-[7px] font-black uppercase text-blue-600 tracking-widest mt-1">Mejor opción</span>}
        </div>
        {isV1Better && <Check className="text-blue-600 flex-shrink-0" size={20} strokeWidth={3} />}
      </div>

      <div className={`md:col-span-4 p-6 flex items-center justify-between transition-all ${isV2Better ? 'bg-blue-50/50' : ''}`}>
        <div className="flex flex-col">
           <span className={`${isSmall ? 'text-[10px]' : 'text-xl'} font-display font-black text-slate-900 tabular-nums`}>
             {val2 === undefined || val2 === null ? 'N/A' : (isPrice ? `$${safeL(val2)}` : `${safeL(val2)}${unit}`)}
           </span>
           {isV2Better && <span className="text-[7px] font-black uppercase text-blue-600 tracking-widest mt-1">Mejor opción</span>}
        </div>
        {isV2Better && <Check className="text-blue-600 flex-shrink-0" size={20} strokeWidth={3} />}
      </div>
    </div>
  );
};
